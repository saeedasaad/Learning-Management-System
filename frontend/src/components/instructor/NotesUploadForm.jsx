...
export default function NotesUploadForm({ courseId, moduleIndex, onNotesAdded }) {
  ...
  return (
    <form onSubmit={handleUpload} className="flex gap-4 mb-6">
      <input type="file" accept=".pdf" onChange={(e) => setFile(e.target.files[0])} className="border p-2 rounded w-1/3"/>
      <button type="submit" className="bg-green-600 text-white px-4 py-2 rounded">Upload Notes</button>
    </form>
  );
}
